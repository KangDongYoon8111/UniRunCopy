...
public class PlatformSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Time.time >= lastSpawnTime + timeBetSpawn)
        {
            ...
            // 다음 배치까지의 시간 간격을 timeBetMin,
            // timeBetMax 사이에서 랜덤 설정
            timeBetSpawn = Random.Range
                (timeBetSpawnMin, timeBetSpawnMax);

            // 배치할 위치의 높이를 yMin과
            // yMax 사이에서 랜덤 설정
            float yPos = Random.Range(yMin, yMax);

            // 사용할 현재 순번의 발판 게임 오브젝트를
            // 비활성화하고 즉시 다시 활성화 이때 발판의
            // Platform 컴포넌트의 OnEnable 메서드가 실행됨
